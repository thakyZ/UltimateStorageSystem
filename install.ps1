...
DynamicParam {
  ...
  [string[]] $Files = @();
  $Files = (Get-ChildItem -LiteralPath $PSScriptRoot -Directory | Where-Object { $Dir = $_; Return ($Null -ne (Get-ChildItem -LiteralPath $_ -File -Recurse -Filter '*.csproj' | Where-Object { (Test-Path -LiteralPath (Join-Path $_.Directory -ChildPath "manifest.json") -PathType Leaf) -and $_.BaseName -eq $Dir.Name })); }).Name
  ...
  [string] $ModCsProj         = (Get-ChildItem -LiteralPath $PSScriptRoot -Recurse -File | Where-Object { $_.Name -eq "$($Mod).csproj" })[0].FullName;
  [string] $ProjectDirectory  = (Get-Item      -LiteralPath $ModCsProj).Directory.FullName;
  [string] $BinDirectory      = (Get-ChildItem -LiteralPath $ProjectDirectory -Recurse -Directory | Where-Object { $_.Name -eq 'bin' }).FullName;
  [string] $BuildOutput       = (Get-ChildItem -LiteralPath $BinDirectory     -Recurse -File -Filter '*.dll' -Depth 3 | Where-Object { $_.BaseName -eq $Mod }).FullName;
  [string] $ManifestFile      = (Get-ChildItem -LiteralPath $ProjectDirectory -Recurse -File -Filter '*.json' | Where-Object { $_.BaseName -eq 'manifest' }).FullName;
  [string] $TranslationDir    = (Get-ChildItem -LiteralPath $ProjectDirectory -Recurse -Directory -Depth 3 | Where-Object { $_.Name -eq 'i18n' }).FullName;
  ...
  If (-not (Test-Path -LiteralPath $Destination -PathType Container)) {
    If ($Null -eq (New-Item -Path $Destination -ItemType Directory -ErrorAction SilentlyContinue)) {
      Throw [Exception]::new('Please specify a destination directory.');
    }
  }

  If (-not $PSBoundParameters.ContainsKey('Destination') -and $Null -eq (Get-ChildItem -LiteralPath $Destination | Where-Object { $_.Name -eq "$($Mod).dll" -or $_.Name -eq 'i18n' })) {
...
#>
  ...
  If (-not [File]::Exists($ModIsProj)) {
    ...
    If ($Items.Count -eq 0) {
      Throw [Exception]::new("Unexpected error occurred, could not find the mod project file. Expected file at `"$($ModIsProj)`" or an equivalent.");
